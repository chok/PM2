...
module.exports = function(God) {
   ...
   * @return object|null
   */
  God.findOneByPID = function(pid) {
    var db = God.clusters_db;
    var arr = [];

    for (var key in db) {
      if (db[key].process.pid && db[key].process.pid === pid) {
        return db[key];
      }
    }
  ...
  God.killProcess = function(pid, cb) {
    ...
    try {
      var p = God.findOneByPID(pid);

      if(p.pm2_env.exec_mode == 'cluster_mode' || p.pm2_env.treekill === true) {
        treekill(pid);
      } else {
